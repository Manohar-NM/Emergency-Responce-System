    ...
    try {
      ...
      if (sosData.alertId) {
        ambulanceTrackingRef.current = setInterval(async () => {
          try {
            const trackResponse = await fetch(`/api/ambulance/track?alertId=${sosData.alertId}`)
            if (trackResponse.ok) {
              const trackData = await trackResponse.json()
              if (trackData.ambulance) {
                setAcceptedHospital({
                  hospitalName: trackData.ambulance.hospitalName,
                  ambulanceStatus: trackData.ambulance.status,
                  eta: trackData.ambulance.eta,
                  latitude: trackData.ambulance.latitude,
                  longitude: trackData.ambulance.longitude,
                })
                  ...
                  <Card className="border-accent/50 bg-accent/5">
                    <CardContent className="pt-4">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Latitude: {currentLocation.latitude.toFixed(4)}, Longitude:{" "}
                        {currentLocation.longitude.toFixed(4)}
                      </p>
                      <p className="text-xs text-muted-foreground">Accuracy: {currentLocation.accuracy.toFixed(0)}m</p>
                    </CardContent>
                  </Card>
                )}
